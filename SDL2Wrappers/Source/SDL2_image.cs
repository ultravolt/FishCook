...
namespace SDL2
{
    public static class Image
    {
        ...
        //(SDL_Surface) Marshal.PtrToStructure(
        //        textSurface ,

        public static SDL.Surface ToSurface(IntPtr intPtr)
        {
            unsafe
            {
                SDL.Surface* s = (SDL.Surface*)intPtr;                
                var surface = new SDL.Surface()
                {
        ...
        [DllImport(nativeLibName, EntryPoint = "IMG_InvertAlpha", CallingConvention = CallingConvention.Cdecl)]
